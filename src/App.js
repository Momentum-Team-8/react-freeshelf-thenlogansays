...
const App = () => {
  const [books] = useState([
    {
      ...
      author: 'Kyle Simpson',
      url: 'https://github.com/getify/You-Dont-Know-JS/blob/2nd-ed/get-started/images/cover.png?raw=true',
  ...
  return (
    <main>
      ...
      {books.map((book, idx) => {
        return (
          <BookCard coverImgUrl={book.coverImageUrl} title={book.title} author={book.author} shortDescription={book.shortDescription} url={book.url} publisher={book.publisher} publicationDate={book.publicationDate} detailedDescription={book.detailedDescription} key={idx} id={idx} />
        )
      })}
    </main>
  )
...
export const BookCard = (props) => {
  ...
  return (
    <div id={id} className='card'>
      <img src={coverImageUrl} alt='Cover Art' />
      <h2>{title}</h2>
      <h3>{author}</h3>
      <p>{shortDescription}</p>
      <button onClick={handleExpand}>{expand ? 'Less' : 'More'}</button>
      {expand && (
        <>
          <a href={url}>{title}</a>
          <p>{publisher}</p>
